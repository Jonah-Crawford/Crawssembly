...
impl WebCpu {
        ...
        match device {
            ...
            0x0 => match command {
                ...
                0x4 => {
                    if self.regs[238] != 0 {
                        self.output
                            .push_str(&format!("Crawssembly Error Code {}", self.regs[238]));
                    }
                }
    ...
    fn execute(
        ...
        loop {
            if tick >= max_ticks {
                return Err(format!(
                    "Stopped after {max_ticks} ticks. Possible infinite loop. Consider downloading Crawssembly for infinite runtime."
                ));
            }
    ...
    fn step(&mut self, d: Decoded, pc: i32, input: &[i32], looped: bool) -> (i32, bool) {
        ...
        match d.op5 {
            ...
            0b00111 => {
                ...
                if t >= 0 { (t, false) } else { (pc + 1, false) }
            }
