...
fn run_bench(program: &[Decoded], iters: u64) {
    ...
    let cpu_mhz = sys.cpus().first().map(|cpu| cpu.frequency()).unwrap_or(0) as f64;
    ...
    println!(
        "BENCH runs={} total_ticks={} time={:.6}s",
        iters, total_ticks, dt
    );
    println!(
        "BENCH throughput:  {:.3} Hz = ~{:.3} MHz",
        hz,
        hz / 1_000_000.0
    );
    ...
    println!(
        "Estimated vm cost: {:.2} host CPU cycles / instruction",
...
fn write_audio(
    ...
    for frame in output.chunks_mut(channels) {
        ...
        if let Ok(mut speech) = speech_samples.lock()
            && let Some(speech_sample) = speech.pop_front()
        {
...
impl Cpu {
    ...
    fn execute_interactive(
        ...
        if plain {
            let _ = execute!(stdout, ResetColor, cursor::Show, DisableMouseCapture);
        } else {
            let _ = execute!(
                stdout,
                ResetColor,
        ...
        if !self.output_buffer.is_empty() {
            ...
            if let Err(e) = fs::write(output_path, &self.output_buffer) {
        ...
        if show_stats {
            ...
            println!(
                "Done: ticks={} time={:.10}s 0xFF={}",
                tick, dt, self.regs[0xFF]
            );
            println!(
                "Clock speed was revealed to be {:.3} Hz = ~{:.3} MHz",
                hz,
                hz / 1_000_000.0
            );
        ...
        match device {
            ...
            0x5 => {
                ...
                match command {
                    ...
                    // toggle
                    0x6 => {
                        speakers[self.speaker_channel].enabled =
                            !speakers[self.speaker_channel].enabled;
                    }
    ...
    fn speech_speak(&mut self) {
        ...
        for n in 0..samples {
            ...
            let source = voiced * (1.0f32 - self.speech_noise) + noise * self.speech_noise;

            let y1 = self
                .speech_r1
                .process(source, self.speech_f1, 90.0f32, sample_rate)
                * 0.75f32;
            let y2 = self
                .speech_r2
                .process(source, self.speech_f2, 120.0f32, sample_rate)
                * 0.45f32;
            let y3 = self
                .speech_r3
                .process(source, self.speech_f3, 180.0f32, sample_rate)
                * 0.25f32;
    ...
    fn write_reg(&mut self, dst: usize, value: i32) {
        ...
        if dst == 0xEF
            && let Some(c) = char::from_u32(value as u32)
        {
